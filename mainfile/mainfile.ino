...
void rxCallback(uint8_t *buffer, uint8_t len)
{
  ...
  if (buffer[0] == 'l' && buffer[1] == 'e' && buffer[2] == 'v' && buffer[3] == 'e' && buffer[4] == 'l'){

    uart.print("Force :");
    uart.print(String(FSRReading));
    if (FSRReading >= 30){
      uart.print("Cup full");
    }

    if (FSRReading < 30 | FSRReading == 0){
      uart.print("Cup almost empty");
    }
  }
  if (buffer[0] == 't' && buffer[1] == 'i' && buffer[2] == 'p'){
  ...
  if (buffer[0] == 'w' && buffer[1] == 'a' && buffer[2] == 't' && buffer[3] == 'e' && buffer[4] == 'r'){
    ...
    if (isEmpty != 1) {
      ...
      if (FSRReading < final_force){
        ...
        delay_time = (3.145926 - theta)/rotation_speed - 500;
        delay(500);
...
////////////////////////// Humid //////////////////////////////////////
  if (buffer[0] == 'h' && buffer[1] == 'u' && buffer[2] == 'm' && buffer[3] == 'i' && buffer[4] == 'd' && buffer[5] == 'i' && buffer[6] == 't' && buffer[7] == 'y'){
   ...
   // if (humid == 0){
    ...
    if (relativeHumidity < 25 | relativeHumidity == 0 | (relativeHumidity == 25)){
      uart.print("Feels dry.");
    }
    if (relativeHumidity > 60 | relativeHumidity == 60){
      uart.print("Feels wet.");
    }
    if (relativeHumidity < 60 && relativeHumidity > 25){
      uart.print("Feels comfy.");
    }
    uart.print("Relative Humidity: ");
    uart.print(relativeHumidity);
  }
/*
  if (humid == 3){
